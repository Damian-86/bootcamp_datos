...
from (select company_id, 
      ...
      dense_rank() over(      -- maneja empates
      partition by company_id -- reinicia el ranking por empresa
      order by sum(amount) desc) ranking -- calcualamos columna con funcion ventana
from transaction 
group by company_id, date(timestamp)) tabla_rankeada 
where ranking <= 5; -- top 5 por empresa
